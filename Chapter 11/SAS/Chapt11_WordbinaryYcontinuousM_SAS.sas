/*Word Data with binary y, 
*/

Data a;
input id x m y r gender verbal;
xnew=x;
x=xnew;
xm=x*m;
If Y gt 13 then ybin=1;
if y le 13 then ybin=0;
if m gt 6 then mbin=1;
if m le 6 then mbin=0;
mbinx=mbin*x;
cards;
1 1 9 10 1 1 5 
2 1 9 15 7 1 5 
...
%mediation(data=data,yvar=ybin,avar=x,mvar=m,cvar=,a0=0,a1=1,m=5,nc=0,yreg=logistic,mreg=linear,
...
%mediation(data=data,yvar=ybin,avar=x,mvar=m,cvar=,a0=0,a1=1,m=5,nc=0,yreg=loglinear,mreg=linear,
interaction=true,casecontrol=,output=full,c=,boot=false);
run;
