...
interaction=,casecontrol=false,output=reduced,c=,boot=false);
...
%mend;
%mediation(data=data,yvar=y,avar=x,mvar=m,cvar=,
a0=0,a1=1,m=5,nc=0,yreg=linear,mreg=linear,interaction=false,casecontrol=,
output=full,c=,boot=false);

proc sort data=data; by x;
proc means data=data; var m;
proc means data=data; var x m y gender; by x;
run;
